...
Kafka/Redpanda được dùng để mô phỏng event streaming. Apache Kafka hỗ trợ publish/subscribe event streams, lưu trữ streams bền vững và xử lý streams khi chúng xảy ra hoặc xử lý lại về sau.
...
- Kafka/Redpanda nhận event stream.
...
Dữ liệu được đọc liên tục từ Kafka/Redpanda nhưng được xử lý theo các batch nhỏ. Latency mục tiêu là khoảng 1–5 phút, phù hợp với dashboard retail/payment.
...
- Pipeline ingest được event stream từ Kafka/Redpanda vào Bronze.
...
Để giải quyết, project thiết kế một nền tảng Lakehouse gồm Kafka/Redpanda, Bronze, Silver, Gold, Delta Lake, benchmark, Airflow và monitoring.

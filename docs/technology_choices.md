...
Project chạy trước trên máy cá nhân bằng Docker Compose, MinIO, Apache Kafka KRaft, Spark local, PostgreSQL, Airflow và Streamlit.
...
|---|---|
| Mô phỏng event stream, duplicate, retry | Apache Kafka KRaft |
...
```text
Kafka → Bronze → Silver → Gold → Delta Lake → Benchmark → Airflow → Monitoring
```
...
|---|---|
| Apache Kafka KRaft | event được publish/consume, có topic, offset metadata |
...
| Apache Kafka KRaft | Amazon MSK | Pub/Sub hoặc Managed Kafka |
...
Project đã chọn Apache Kafka KRaft làm Kafka event-streaming layer. Producer gửi synthetic retail/payment events vào topic `retail-payment-events`; các consumer ở những tuần sau sẽ đọc theo topic, partition và offset để ghi Bronze.

Kafka giúp project mô phỏng các vấn đề thực tế của pipeline gần thời gian thực:

- event đến liên tục;
- duplicate do retry hoặc application gửi có chủ đích;
- late events;
- malformed events;
- replay theo offset;
- ingestion metadata;
...
### 5.2. Vì sao chọn Apache Kafka

Apache Kafka phù hợp với project vì nó cung cấp topic, partition, offset, retention và producer/consumer API phổ biến trong hệ sinh thái Data Engineering. Các khái niệm này gắn trực tiếp với yêu cầu audit, replay và trace source metadata từ Bronze về lại event stream.

Project không dùng runtime Kafka-compatible thay thế trong local stack. Docker Compose hiện dùng image Kafka chính thức và các lệnh Kafka CLI trong container.

### 5.3. Vì sao dùng KRaft

Kafka được chạy ở KRaft mode để Kafka tự quản lý metadata mà không cần ZooKeeper. Cấu hình local dùng:
...
Thiết kế này giảm số service cần vận hành trong môi trường học tập và phát triển local.

### 5.4. Thiết kế local một broker

Local stack dùng một node Kafka combined broker/controller để giảm độ phức tạp khi chạy trên máy cá nhân.

Trade-off quan trọng:

- replication factor là `1`, nên không có high availability;
- broker/controller chạy chung chỉ phù hợp development;
- mất node Kafka local đồng nghĩa mất khả năng phục vụ event stream cho đến khi service khởi động lại;
- production cần tách controller/broker và dùng nhiều node.

### 5.5. Topic, partition, offset và retention
...
| Kafka record key | `order_id` |

`order_id` được dùng làm Kafka record key để các event cùng đơn hàng thường đi vào cùng partition. Kafka chỉ đảm bảo ordering trong cùng một partition, không đảm bảo ordering trên toàn topic.
...
### 5.6. Listener trong host và Docker network

Kafka local có ba cách truy cập chính:

| Vị trí client | Bootstrap server |
|---|---|
| Python producer chạy trên host | `localhost:9092` |
| Service trong Docker network | `kafka:19092` |
| Kafka CLI chạy trong container Kafka | `localhost:19092` |

Không dùng IP container cố định vì container có thể được tạo lại và đổi IP.

### 5.7. Trade-offs và local limitations

Kafka KRaft local giúp project có topic, partition, offset, retention và replay semantics để kiểm thử pipeline. Đổi lại, cấu hình Tuần 1 không phải cấu hình production:

- một broker;
- replication factor `1`;
- combined broker/controller;
- không high availability;
- không có consumer group nghiệp vụ trong Tuần 1;
- Kafka → Bronze thuộc scope Tuần 2.
...
Python Producer sinh synthetic retail/payment events và gửi vào Kafka.
...
| Event streaming | Apache Kafka KRaft | Mô phỏng event stream | MVP |
...
### 18.2. Vì sao dùng Apache Kafka KRaft?

Apache Kafka KRaft được dùng làm event streaming layer. Nó giúp project tạo được dữ liệu đến liên tục, duplicate do retry, late events và replay theo offset. Nếu chỉ đọc file batch, project sẽ thiếu các vấn đề thực tế của pipeline near real-time.
...
2. Apache Spark Structured Streaming Programming Guide — https://spark.apache.org/docs/3.5.8/structured-streaming-programming-guide.html
